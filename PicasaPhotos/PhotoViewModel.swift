...
class PhotoViewModel {
    ...
    init(photo: Photo) {
        ...
        photo.rx.observe(String.self, "id")
            .map { _ in self.photo.id }.bind(to: self.id)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "title")
            .map { _ in self.photo.title }.bind(to: self.title)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "access")
            .map { _ in self.photo.access }.bind(to: self.access)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "imageURL")
            .map { _ in self.photo.imageURL }.bind(to: self.imageURL)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "thumbnailURL")
            .map { _ in self.photo.thumbnailURL }.bind(to: self.thumbnailURL)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "geoLocation")
            .map { _ in self.photo.geoLocation }.bind(to: self.geoLocation)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "published")
            .map { _ in self.photo.published }.bind(to: self.published)
            .addDisposableTo(disposeBag)
        photo.rx.observe(String.self, "updated")
            .map { _ in self.photo.updated }.bind(to: self.updated)
            .addDisposableTo(disposeBag)
    }
}
